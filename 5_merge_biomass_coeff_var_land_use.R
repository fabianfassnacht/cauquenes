## R-Script - merging coefficient of variation maps using the land cover data 
...
# jump to folder containing coefficient of variation maps
# of the four vegetation types and load maps

setwd("/home/fabian/Fondecyt/5_outputs/5_maps")

map_bn <- stack("CEFF_VAR_biom_bn_100ter.tif")
map_pin <- stack("CEFF_VAR_biom_pine_100ter.tif") 
map_mat <-  stack("CEFF_VAR_biom_mat_100ter.tif")
map_prad <- stack("CEFF_VAR_biom_prad_100ter.tif")
...
# resample coefficient of variation maps to match
...
for (i in 1:length(res_v)) {
  ...
  if (fcl_v[i] %in% c(0,1,2,4,5,6,10,11,12)){
    ...
    # if pixel of land-cover is native forest, assign native forest coefficient of variation
    } else if (fcl_v[i] == 3) {
      
      res_v[i] <- bm_df[i,1]
    # if pixel of land-cover is shrubland, assign shrublandt coefficient of variation  
    } else if (fcl_v[i] == 7) {
      
      res_v[i] <- bm_df[i,2]
    # if pixel of land-cover is pine forest, assign pine forest coefficient of variation
    } else if (fcl_v[i] == 8) {
...
writeRaster(res_raster, filename = "final_bm_coeffvar_map_luc.tif", format="GTiff")
