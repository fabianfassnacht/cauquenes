...
#save(biomasa_prad_df, file="biomasa_prad.RData")
load("biomasa_prad.RData")
#######################
# BN
...
#save(biomasa_bn_df, file="biomasa_bn_df.RData")
load("biomasa_bn_df.RData")
...
save(biomasa_PINO_df, file="biomasa_pino_df.RData")
load("biomasa_pino_df.RData")
...
#save(biomasa_matg_df, file="biomasa_matg_df.RData")
load("biomasa_matg_df.RData")
...
png(filename = "pradera_vsurf_sctter.png", width=1100, height=1100, res=250)
  ...
  # check results
  plot(biomasa_prad_df[,2], rfm2_pr$predicted, ylim=c(0,8), xlim=c(0,8), pch=16, col=rgb(0, 0, 1, 0.25),
...
source("/home/fabian/Fondecyt/1_R_code/codes_final/9_gam_nice_plots.R")
...
gam_m_pr <- mgcv::gam(AGB_ton_ha ~  
                  s(B12_ver) + 
                  s(B5_inv),                
                  data = gam_prad, method="REML", select=TRUE)#, family = "Gamma")
...
setwd("/home/fabian/Fondecyt/5_outputs/2_plots")
png(width = 1700, height= 800, res=250, filename = "prad_gam_resp_curv_nice.png")
gam_nice_plot(gam_m_pr, gam_prad)
dev.off()
...
# check results
plot(biomasa_matorralg_df2[,2], rfm_matg3$predicted, ylim=c(0,20), xlim=c(0,20), pch=16, col=rgb(0, 0, 1, 0.25), 
...
gam_matg <- gam_matg[,c(1,2,5,7,8)]

colnames(gam_matg)

edfval <- str_detect(names(gam_m_matg$edf), z[ip3])

test
round(sum(gam_m_matg$edf[test]),2)
...
gam_m_matg <- mgcv::gam(AGB_ton_ha ~  
                        ...
                        s(ProfCurv),
                        #s(B08a_inv_3x3_var),
                      data = gam_matg, method="REML", select=TRUE)#, family = "Gamma")
...
setwd("/home/fabian/Fondecyt/5_outputs/2_plots")
png(width = 1700, height= 1700, res=250, filename = "matg_gam_resp_curv_nice.png")
gam_nice_plot(gam_m_matg, gam_matg)
dev.off()
...
png(filename = "bn_vsurf_sctter.png", width=1100, height=1100, res=250)
  ...
  # check results
  plot(biomasa_bn_df[,2], rfm3_bn$predicted, ylim=c(0,400), xlim=c(0,400),  pch=16, col=rgb(0, 0, 1, 0.25),
...
setwd("/home/fabian/Fondecyt/5_outputs/2_plots")
png(width = 1700, height= 1700, res=250, filename = "bn_gam_resp_curv_nice.png")
gam_nice_plot(gam_bn_pr, gam_bn, 2, 2)
dev.off()
...
png(filename = "pine_vsurf_sctter.png", width=1100, height=1100, res=250)
  ...
  # check results
  plot(biomasa_PINO_df[,2], rfm2_pin$predicted, ylim=c(0,400), xlim=c(0,400), pch=16, col=rgb(0, 0, 1, 0.25),
...
setwd("/home/fabian/Fondecyt/5_outputs/2_plots")
png(width = 1700, height= 1700, res=250, filename = "pin_gam_resp_curv_nice.png")
gam_nice_plot(gam_pin_pr, gam_pin, 2, 2)
dev.off()
